...
export async function generateDailyReport(input: Omit<BuildReportInput, "reportType" | "costUsage"> = {}) {
  if (config.aiMode === "manual") {
    return generateMarketManualGptPack(input.date, "postmarket");
  }
  const costUsage = await new CostGuard().readUsage();
  const report = new ReportEngine().build({ ...input, reportType: "postmarket", costUsage });
  return {
    report,
